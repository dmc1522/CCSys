/****
This SQL script was generated by the Configure Data Synchronization
dialog box. This script contains statements that create the
change-tracking columns, deleted-items table, and triggers on the
server database. These database objects are required by Synchronization
Services to successfully synchronize data between client and server
databases. For more information, see the
‘How to: Configure a Database Server for Synchronization’ topic in Help.
****/


IF @@TRANCOUNT > 0
set ANSI_NULLS ON 
set QUOTED_IDENTIFIER ON 

GO
BEGIN TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Users] 
ADD [LastEditDate] DateTime NULL CONSTRAINT [DF_Users_LastEditDate] DEFAULT (GETUTCDATE()) WITH VALUES
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Users] 
ADD [CreationDate] DateTime NULL CONSTRAINT [DF_Users_CreationDate] DEFAULT (GETUTCDATE()) WITH VALUES
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[Users_Tombstone]')) 
BEGIN 
CREATE TABLE [dbo].[Users_Tombstone]( 
    [userID] Int NOT NULL,
    [DeletionDate] DateTime NULL
) ON [PRIMARY] 
END 

GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Users_Tombstone] ADD CONSTRAINT [PKDEL_Users_Tombstone_userID]
   PRIMARY KEY CLUSTERED
    ([userID])
    ON [PRIMARY]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[Users_DeletionTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[Users_DeletionTrigger] 

GO
CREATE TRIGGER [dbo].[Users_DeletionTrigger] 
    ON [dbo].[Users] 
    AFTER DELETE 
AS 
SET NOCOUNT ON 
UPDATE [dbo].[Users_Tombstone] 
    SET [DeletionDate] = GETUTCDATE() 
    FROM deleted 
    WHERE deleted.[userID] = [dbo].[Users_Tombstone].[userID] 
IF @@ROWCOUNT = 0 
BEGIN 
    INSERT INTO [dbo].[Users_Tombstone] 
    ([userID], DeletionDate)
    SELECT [userID], GETUTCDATE()
    FROM deleted 
END 

GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[Users_UpdateTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[Users_UpdateTrigger] 

GO
CREATE TRIGGER [dbo].[Users_UpdateTrigger] 
    ON [dbo].[Users] 
    AFTER UPDATE 
AS 
BEGIN 
    SET NOCOUNT ON 
    UPDATE [dbo].[Users] 
    SET [LastEditDate] = GETUTCDATE() 
    FROM inserted 
    WHERE inserted.[userID] = [dbo].[Users].[userID] 
END;
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[Users_InsertTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[Users_InsertTrigger] 

GO
CREATE TRIGGER [dbo].[Users_InsertTrigger] 
    ON [dbo].[Users] 
    AFTER INSERT 
AS 
BEGIN 
    SET NOCOUNT ON 
    UPDATE [dbo].[Users] 
    SET [CreationDate] = GETUTCDATE() 
    FROM inserted 
    WHERE inserted.[userID] = [dbo].[Users].[userID] 
END;
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
COMMIT TRANSACTION;
